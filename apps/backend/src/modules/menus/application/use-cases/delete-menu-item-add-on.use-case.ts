...
import { AddOnDeletedEvent } from '../../domain/events/menu.events';
import {
  assertActorCanManageMenu,
  resolveMenuManagementActorId,
} from '../services/assert-actor-can-manage-menu';
import { DeleteMenuItemAddOnCommand } from '../dto/menu-item-add-on.commands';

@Injectable()
export class DeleteMenuItemAddOnUseCase {
  constructor(
  ...
  async execute(command: DeleteMenuItemAddOnCommand): Promise<void> {
    ...
    await this.unitOfWork.execute(async () => {
      await this.addOnRepository.softDelete(addOnId, now);
    });

    const actorId = resolveMenuManagementActorId(command.actor);
    await this.eventPublisher.publish(
      new AddOnDeletedEvent(

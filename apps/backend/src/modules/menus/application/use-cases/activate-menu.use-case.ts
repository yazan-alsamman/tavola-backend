...
import { MenuActivatedEvent } from '../../domain/events/menu.events';
import {
...
@Injectable()
export class ActivateMenuUseCase {
  constructor(
  ...
  async execute(command: SetMenuActiveStateCommand): Promise<MenuResult> {
    ...
    const activated = menu.activate(now);

    await this.unitOfWork.execute(async () => {
      await this.menuRepository.update(activated);
    });

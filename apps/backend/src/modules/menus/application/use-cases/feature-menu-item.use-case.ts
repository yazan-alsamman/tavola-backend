...
import { MenuItemFeaturedEvent } from '../../domain/events/menu.events';
import {
...
/** ADR-032: `isFeatured: false -> true`. */
@Injectable()
export class FeatureMenuItemUseCase {
  constructor(
  ...
  async execute(command: SetMenuItemFeaturedCommand): Promise<MenuItemResult> {
    ...
    const updated = item.feature(now);

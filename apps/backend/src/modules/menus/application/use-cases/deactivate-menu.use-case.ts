...
import { MenuDeactivatedEvent } from '../../domain/events/menu.events';
import {
...
import { SetMenuActiveStateCommand } from '../dto/menu.commands';
import { MenuResult } from '../dto/menu.result';

@Injectable()
export class DeactivateMenuUseCase {
  constructor(
  ...
  async execute(command: SetMenuActiveStateCommand): Promise<MenuResult> {
    ...
    const deactivated = menu.deactivate(now);

    await this.unitOfWork.execute(async () => {
      await this.menuRepository.update(deactivated);
    });

    const actorId = resolveMenuManagementActorId(command.actor);
    await this.eventPublisher.publish(
      new MenuDeactivatedEvent(
    ...
    return toMenuResult(deactivated);
  }
}

...
import { OptionGroupUpdatedEvent } from '../../domain/events/menu.events';
import {
...
import { UpdateMenuItemOptionGroupCommand } from '../dto/menu-item-option-group.commands';
import { MenuItemOptionGroupResult } from '../dto/menu-item-option-group.result';

@Injectable()
export class UpdateMenuItemOptionGroupUseCase {
  constructor(
  ...
  async execute(command: UpdateMenuItemOptionGroupCommand): Promise<MenuItemOptionGroupResult> {
    ...
    const updated = group.update(command.content, now);

    await this.unitOfWork.execute(async () => {
      await this.optionGroupRepository.update(updated);
    });
    ...
    return toMenuItemOptionGroupResult(updated);
  }
}

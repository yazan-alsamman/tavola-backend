...
import { MenuItemUnfeaturedEvent } from '../../domain/events/menu.events';
import {
...
import { SetMenuItemFeaturedCommand } from '../dto/menu-item.commands';
import { MenuItemResult } from '../dto/menu-item.result';

/** ADR-032: `isFeatured: true -> false`. */
@Injectable()
export class UnfeatureMenuItemUseCase {
  constructor(
  ...
  async execute(command: SetMenuItemFeaturedCommand): Promise<MenuItemResult> {
    ...
    const updated = item.unfeature(now);

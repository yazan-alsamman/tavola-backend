...
import { CancelSubscriptionCommand } from '../dto/cancel-subscription.command';
import { SubscriptionResult } from '../dto/subscription.result';

/**
 * PlatformAdmin-only (D9). `Active -> Cancelled` only. Terminal - not
 * automatically reactivated (D8). Audit entry derived automatically by
...
export class CancelSubscriptionUseCase {
  ...
  async execute(command: CancelSubscriptionCommand): Promise<SubscriptionResult> {
  ...
  private async cancel(command: CancelSubscriptionCommand): Promise<SubscriptionResult> {
    ...
    await this.unitOfWork.execute(async () => {
      applied = await this.subscriptionRepository.updateIfStatus(cancelled, existing.status);
    });
    if (!applied) {
    ...
    await this.expirationScheduler.cancelExpiration(cancelled.subscriptionId.value);

    await this.eventPublisher.publish(
      new SubscriptionCancelledEvent(
        this.idGenerator.generate(),
        {
          subscriptionId: cancelled.subscriptionId.value,
          organizationId: command.organizationId,
    ...
    return toSubscriptionResult(cancelled);
  }
}

...
describe('DeleteTableUseCase', () => {
  ...
  async function build() {
    ...
    const useCase = new DeleteTableUseCase(
      tableRepository,
      branchRepository,
      restaurantRepository,
      new FixedClock(fixedNow),
      new SequentialIdGenerator(['aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa']),
      eventPublisher,
    );
    return { useCase, tableRepository, eventPublisher };
  }

  it('soft-deletes the table (not a hard delete)', async () => {
    const { useCase, tableRepository } = await build();

    await useCase.execute({ actor: baseActor(), tableId });

    expect(await tableRepository.findById(TableId.create(tableId))).toBeNull();
  });
  ...
  it('is not idempotent - deleting twice throws TableNotFoundException the second time', async () => {
    const { useCase } = await build();
  ...
  it('publishes exactly one TableDeletedEvent', async () => {
    const { useCase, eventPublisher } = await build();

    await useCase.execute({ actor: baseActor(), tableId, correlationId: 'corr-1' });

    expect(eventPublisher.events).toHaveLength(1);
    expect(eventPublisher.events[0]).toBeInstanceOf(TableDeletedEvent);
  });
});

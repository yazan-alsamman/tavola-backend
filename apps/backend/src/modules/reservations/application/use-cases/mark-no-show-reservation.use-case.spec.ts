import { MarkNoShowReservationUseCase } from './mark-no-show-reservation.use-case';
...
describe('MarkNoShowReservationUseCase', () => {
  ...
  function employeeActor(overrides?: { branchIds?: string[] }) {
    return {
      ...
      permissions: ['reservations:noshow'],
  ...
  it('marks an Approved reservation NoShow once the scheduled time has passed, releasing the Table', async () => {
  ...
  it('publishes ReservationNoShow with the acting employeeId, never reservations:approve', async () => {
    ...
    const event = eventPublisher.events[0] as ReservationNoShowEvent;
    expect(event).toBeInstanceOf(ReservationNoShowEvent);
  ...
  ])('rejects marking NoShow on a reservation that is %s (not Approved)', async (status) => {
    const { useCase, reservationRepository } = await build(startTime);

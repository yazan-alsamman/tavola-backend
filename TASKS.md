...
# Public Working Hours & Restaurant Phone Privacy — Implementation & Verification Report

**Status: IMPLEMENTED, LIVE VERIFIED (2026-08-09).** Targeted, additive/corrective session on top of the existing Discovery Module (Phase 15.5): (1) customers can now see restaurant/branch working hours when browsing publicly, and (2) the pre-existing accidental exposure of `Branch.phone` through the public Discovery surface is closed. No new Prisma model/migration, no new endpoint, no tenancy/architecture change - both corrections extend the already-existing `WorkingHours`/`BranchWorkingHours` tables and the already-existing `GET /discovery/restaurants[/:restaurantId[/branches[/:branchId]]]` routes.

**Root cause (phone):** `DiscoveryController`'s branch routes (`GET .../branches`, `GET .../branches/:branchId`) reused the private `BranchResponseDto`/`BranchListResponseDto` verbatim via `toDiscoveryBranchResponse` (`discovery-response.mapper.ts`), which copied `result.phone` straight through - the exact same DTO the Owner/Admin-only management endpoint (`GET /restaurants/:restaurantId/branches/:branchId`) legitimately returns it on. `Restaurant` itself carries no phone/contact field, so only the Branch surface was affected.

**Working hours source of truth:** the existing relational `WorkingHours` (Restaurant-level default, Phase 4.3) and `BranchWorkingHours` (Branch-level override, Phase 5.2) tables - no new table, `Branch.openingHours` (pre-existing dead `Json` column) remains untouched/unused. No precedence/fallback between the two levels is invented here (`DATABASE_SCHEMA.md`'s "Branch Working Hours" section still documents that as an open question for a future consumer) - the Restaurant-level default surfaces on the Restaurant routes, the Branch-level override surfaces on the Branch routes, independently.

**Fix (both):** new dedicated customer-safe DTOs in `modules/discovery/presentation/dto/` - `WorkingHoursEntryPublicResponseDto` (shared entry shape, `dayOfWeek`/`openingTime`/`closingTime`/`breakStartTime`/`breakEndTime`, no audit timestamps, matching Phase 15.5's own `FloorPlanPublicResponseDto`/`TablePublicResponseDto` precedent - D11), `RestaurantPublicResponseDto` (`RestaurantResponseDto` + `workingHours`), `BranchPublicResponseDto` (every `BranchResponseDto` field except `phone`, + `workingHours`), `BranchPublicListResponseDto`. `discovery-response.mapper.ts`'s `toDiscoveryBranchResponse` now builds the response object field-by-field with no `phone` key ever assigned - a structural omission, not a hidden TypeScript field. Two new batched cross-module use cases supply the annotation with no N+1: `ListWorkingHoursByRestaurantIdsUseCase` (Restaurants module, exported) and `ListBranchWorkingHoursByBranchIdsUseCase` (Branches module, exported), each backed by one new `findAllBy...Ids` repository method (`IN (...)` query) added to the existing `WorkingHoursRepository`/`BranchWorkingHoursRepository`. Neither new use case resolves `RESTAURANT_REPOSITORY`/`BRANCH_REPOSITORY` itself (both are `DIRECT_TENANT_OWNED_MODELS`-adjacent/enforced and would throw under Discovery's unauthenticated, no-tenant-context call path) - callers (Discovery's six restaurant/branch use cases) already resolved visibility via `DiscoveryReaderPort` first, mirroring `ListRestaurantIdsWithActiveOfferUseCase`'s exact precedent. `DiscoveryModule` now imports `RestaurantsModule`/`BranchesModule` for these two use cases only - `PrismaDiscoveryReader`'s raw-Prisma bypass for Restaurant/Branch/FloorPlan/Table itself is unchanged.

**Files created:** `apps/backend/src/modules/restaurants/application/use-cases/{list-working-hours-by-restaurant-ids.use-case.ts, list-working-hours-by-restaurant-ids.use-case.spec.ts}`; `apps/backend/src/modules/branches/application/use-cases/{list-branch-working-hours-by-branch-ids.use-case.ts, list-branch-working-hours-by-branch-ids.use-case.spec.ts}`; `apps/backend/src/modules/discovery/presentation/dto/{working-hours-entry-public.response.dto.ts, restaurant-public.response.dto.ts, branch-public.response.dto.ts, branch-public-list.response.dto.ts}`; `apps/backend/src/modules/discovery/presentation/controllers/discovery-response.mapper.spec.ts`.

**Files modified:** `apps/backend/src/modules/restaurants/{domain/repositories/working-hours.repository.ts, infrastructure/persistence/prisma-working-hours.repository.ts, restaurants.module.ts}`; `apps/backend/src/modules/branches/{domain/repositories/branch-working-hours.repository.ts, infrastructure/persistence/prisma-branch-working-hours.repository.ts, branches.module.ts}`; `apps/backend/src/modules/discovery/{discovery.module.ts, application/use-cases/{list-discoverable-restaurants,get-discoverable-restaurant,list-discoverable-branches,get-discoverable-branch,nearby-restaurants,compare-restaurants}.use-case.ts, presentation/{controllers/{discovery.controller.ts, discovery-response.mapper.ts}, dto/discoverable-restaurant.response.dto.ts}}`; existing discovery use-case specs (constructor signature updates + new assertions); `apps/backend/test/{restaurants/support/in-memory-working-hours.repository.ts, branches/support/in-memory-branch-working-hours.repository.ts, restaurants/prisma-working-hours.integration-spec.ts, branches/prisma-branch-working-hours.integration-spec.ts, discovery/discovery.e2e-spec.ts}`; `apps/backend/postman/TAVLA-API.postman_collection.json` (Discovery folder request descriptions, plus one real example response added per request - `Compare Restaurants`, `Search Nearby Restaurants`, `Search/browse restaurants`, `Get a restaurant by id`, `Get a branch by id`, `Browse branches of a restaurant` - each reflecting the shape this session's live verification proved correct: `workingHours` present, `phone` structurally absent from every Branch example); `docs/{API_GUIDELINES.md, DATABASE_SCHEMA.md}`. **No Prisma schema change, no migration** (`prisma validate`: schema valid, zero edits to `schema.prisma` this session).

**Authorization/regression:** unaffected - every management endpoint (`RestaurantsController`, `BranchesController`, `PlatformAdminRestaurantsController`) is untouched; `docs/API_GUIDELINES.md`'s Discovery entry corrected in place (no ADR changed - the Discovery module's DTO-reuse choice was an implementation detail, not a frozen architectural decision) plus one new paragraph documenting this correction; `docs/DATABASE_SCHEMA.md`'s `WorkingHours`/`BranchWorkingHours`/`Branch` sections gained one note each pointing at the new public consumer, without touching their existing "unresolved precedence" / "dead `openingHours` column" notes.

**Verification**: `tsc --noEmit` clean; ESLint zero warnings/errors on every touched file (a pre-existing, unrelated `eslint .` failure on `coverage/`, `coverage-integration/`, `scripts/*.ts`, `prisma/seed.ts`, and `postman/_rebuild_collection.js` - none of them TypeScript-project members, none touched this session - predates this work); `nest build` clean; `prisma format`/`validate` clean, zero schema edits. Unit tests: the full touched-module suite (Discovery + the two new use cases) is 13/13 suites, 68/68 tests green; the full repository suite is 255/262 suites, 2032/2044 tests green, unchanged before and after the Docker rebuild below - the 7 failing suites (`refresh-session`, `login`, `reset-password`, `list-active-sessions`, `logout-all-devices`, `change-password` use-case specs under `modules/authentication`, and `request-account-deletion.use-case.spec.ts` under `modules/users`) are pre-existing and unrelated: none are files this session touched, `refresh-session`/`login`/etc. are unmodified/committed files already failing before this session started, and the account-deletion spec belongs to a separate, already-in-progress uncommitted feature (Phase 20.x) this session never opened.

**Live Docker/HTTP verification (2026-08-09)**: Docker Desktop was initially unreachable (`docker ps` failed to connect to the daemon - the engine process was not running, `com.docker.service` was `Stopped`); launching `Docker Desktop.exe` brought the engine up within ~10s, after which the project's existing dev stack (5 containers, already provisioned from a prior session) was reachable. Rebuilt the backend image from the current working tree per `ENVIRONMENT_SETUP.md`'s documented command (`docker compose --env-file ../.env.development up -d --build`, run from `apps/backend/docker/`): image id before `590fe6e948d4` (built 2026-08-07) → after `9da5cf31cbfe` (built 2026-08-09), confirmed via `docker inspect tavla-backend-1 --format '{{.Image}}'` matching the new id. All 5 containers healthy (`tavla-postgres-1`, `tavla-redis-1`, `tavla-minio-1` report Docker `healthy`; `tavla-nginx-1` has no declared healthcheck but is `Up` and proxying correctly; `tavla-backend-1` reports Docker `healthy` and its logs show `"Nest application successfully started"` with every Discovery route mapped, no runtime errors). `prisma migrate status` (run from host against the published Postgres port, `DATABASE_URL=postgresql://tavla:tavla_dev_password@localhost:5433/tavla_dev`) reports **"Database schema is up to date!"** (42 migrations, zero drift) - an in-container attempt via `docker exec` failed only due to a `docker exec`-through-Git-Bash path-mangling artifact rewriting the `--schema` flag's path, not a real migration issue; the host-side run is authoritative and unambiguous.

Real HTTP verification through the actual Nginx path (`http://localhost/api/v1/...`, port 80, no bypass): seeded a real Owner/Organization/Subscription directly via Prisma (Owner self-registration is retired per ADR-022 - the same fixture shape `test/helpers/owner-fixture.ts` already uses for e2e), authenticated through the real `POST /auth/login`, then created a real Restaurant + Branch (with a real phone number) and configured real working hours at both the Restaurant and Branch level through the real authenticated management API - all before ever calling a Discovery route. Every assertion below inspected the actual serialized HTTP JSON, recursively, not a TypeScript type:

* **A. Public Restaurant Detail** (`GET /discovery/restaurants/:id`) - `workingHours` present, exactly matching the two configured entries in `dayOfWeek` order (including the `breakStartTime`/`breakEndTime` pair on the second entry); zero `phone`-family keys anywhere in the response, recursively.
* **B. Public Branch Detail** (`GET /discovery/restaurants/:id/branches/:branchId`) - `workingHours` present, exactly matching the one configured Branch-level entry; the branch's real phone value does not appear anywhere in the raw JSON string, and zero `phone`-family keys anywhere, recursively.
* **C. Public Restaurant List** (`GET /discovery/restaurants`) - the seeded restaurant's list item carries `workingHours`; zero `phone`-family keys anywhere in the full list response.
* **D. Nearby Discovery** (`GET /discovery/restaurants/nearby`) - a second, geo-coordinated branch (with its own real phone) was added to make the restaurant qualify; the nearby result carries `workingHours`; zero `phone`-family keys anywhere.
* **E. Restaurant Comparison** (`POST /discovery/restaurants/compare`) - the compared item carries `workingHours`; zero `phone`-family keys anywhere.
* **F. Branch List** (`GET /discovery/restaurants/:id/branches`) - every branch item carries `workingHours`; the real phone value does not appear anywhere in the raw JSON string; zero `phone`-family keys anywhere.
* **G. No Working Hours** - a second Restaurant/Branch pair with no working hours ever configured returns `200` (never an error) with `workingHours: []` at both levels; zero `phone`-family keys.
* **H. Recursive Privacy Check** - performed as an integral, non-optional part of every assertion above (a recursive key-walk over the full parsed JSON body, matching `phone`/`phoneNumber`/`restaurantPhone`/`branchPhone`/`contactPhone`/`mobile` case-insensitively at any depth) - zero hits across all six response shapes.
* **I. Authorized Management Regression** - the same branch's authorized endpoint (`GET /restaurants/:restaurantId/branches/:branchId`, Owner JWT) still returns the real `phone` value unchanged - the privacy fix did not remove operational data from authorized users.

All scratch data (Restaurant/Branch/WorkingHours/BranchWorkingHours/OrganizationMember/Subscription/SubscriptionUsage/Organization/User/DeviceSession/TokenFamily rows) was created and deleted via an ephemeral, git-ignored verification script that was itself deleted after the run - `git status` before and after this verification is identical (confirmed via `git status --porcelain`), no production code, schema, or test file was touched during live verification beyond what the implementation phase above already committed to the working tree.

* **J. Postman** - confirmed: the main collection's Discovery folder requests (list/detail/nearby/compare/branch-list/branch-detail) carry the updated descriptions documenting `workingHours` and phone privacy, added during the implementation phase above; no separate fragment file exists.
* **K. Regression** - re-ran the full unit suite after the live-HTTP verification: identical result, 255/262 suites and 2032/2044 tests green, the same 7 pre-existing/unrelated failures and no new ones.

**IMPLEMENTATION COMPLETE. LIVE VERIFIED.** **PRODUCTION VERIFIED** is not claimed - live verification here targeted the local dev Docker stack (`docker-compose.yml` + `docker-compose.override.yml`, `.env.development`) with a manually rebuilt backend image, not an actual production deployment/release pipeline; no production environment, secrets, or infrastructure were touched or exist to verify against in this session.

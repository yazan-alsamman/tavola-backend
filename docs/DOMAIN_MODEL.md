...
* **Organization lifecycle and ownership transfer (ADR-034, architecture frozen 2026-08-04; Suspend/Reactivate/Transfer implemented Phase 19.1, 2026-08-04; Delete/Restore implemented Phase 19.4, 2026-08-10).** `OrganizationMember.changeRole()` already throws `OwnershipTransferRequiredException`, referencing a transfer path that does not otherwise exist. ADR-034 authorizes: (1) PlatformAdmin-authorized Suspend/Reactivate/Delete/Restore, never cascading to `Restaurant.status`/`deletedAt` (no second writer of that field — reuses ADR-027's own rejected-alternative reasoning); Delete/Restore reuse `Organization.deletedAt` (no new schema) exactly as Restaurant's own Delete/Restore already do — Delete has no precondition (re-applies harmlessly if already deleted), Restore rejects (409) an Organization that is not currently deleted, and restoring never also reactivates a Suspended Organization (`status` and `deletedAt` are independent axes); (2) a **narrow, PlatformAdmin-only emergency ownership transfer**, reusing `OrganizationMembershipPolicy.assertSingleActiveOwner` unchanged, resolving `OwnershipTransferRequiredException`'s standing precondition. **Full self-service Organization management** (Owner-initiated Invite/Change Role/Remove/Transfer via an `OrganizationMemberGuard`-gated controller) **remains explicitly out of scope** — a real, separate, pre-existing gap, deliberately not absorbed into ADR-034.
...
* **FloorPlanArea (ADR-040)** is a child entity of FloorPlan — two levels below the Branch Aggregate Root — representing the **concurrent** dining areas (halls) inside ONE layout: "الصالة الرئيسية"/Main Hall, "للضيوف"/Guests, a terrace. Every area of a FloorPlan is live at the same time; the entity has no activation concept of its own, and `FloorPlan.isActive` is never overloaded to express it. An area carries a required presentation `color` (`#RRGGBB`, normalized uppercase) and a `sortOrder` for editor tab order. Its invariants:
  1. An area belongs to exactly one FloorPlan and **never migrates** between layouts — the tables inside it are positioned relative to that one specific layout, so moving the area would silently invalidate all of them. `Update Area` replaces only name, colour and order.
  2. Area names are unique per FloorPlan among non-deleted areas; a soft-deleted area releases its name for reuse. Names are trimmed before comparison and storage.
  3. An area cannot be deleted while any non-soft-deleted Table still references it — the delete is rejected, never silently reassigning or orphaning those tables (the same rule FloorPlan already has for `floorPlanId`).
  4. A FloorPlan with **zero** areas is fully valid and fully configured: its tables simply sit on the layout itself.
* **Table area membership and colour (ADR-040):** `Table.floorPlanAreaId` is nullable — `null` means "placed on the layout itself, in no named area", which is the state of every table predating this decision and the permanent state of a branch that never divides its layout. When set, it must name an area of the **table's own** FloorPlan; a cross-plan assignment is impossible, enforced both in the application layer and by a composite foreign key (see DATABASE_SCHEMA.md). `Table.color` is an optional `#RRGGBB` override whose `null` means "inherit the area's colour", never "no colour" — the server never materializes the inherited value. Both are presentation metadata only, exactly like `shape`: they participate in no reservation, capacity, availability or merge/split rule.
...
* **Acquisition recording never blocks Reservation approval** (2026-08-09 product/architecture decision, see `DATABASE_SCHEMA.md`'s Customer Acquisitions notes for the full "no-configured-operating-currency" case). A Restaurant with no operating currency configured at all silently skips acquisition recording rather than failing the approval — ADR-033 §17's fail-closed rule still applies unchanged once a currency is configured but no rule matches it.
...
* **Move Table (Phase 6.2 architecture decision; Merge/Split guard per ADR-026; area reconciliation per ADR-040):** a dedicated Domain Action, not a generic resource update - it reassigns an existing Table's `floorPlanId` to a different FloorPlan within the same Branch, and changes nothing else (`branchId`, `tableNumber`, `capacity`, `shape`, `color`, position/rotation/dimensions, `status`, and `mergeGroupId`/`isMergePrimary` are all untouched by this operation). **ADR-040 adds the one exception that is inseparable from `floorPlanId`:** the move also sets `floorPlanAreaId` to an area of the **target** plan (supplied by the caller) or to `null`. The previous plan's area is never carried over, because an area belongs to exactly one FloorPlan. Cross-branch and cross-restaurant movement are not allowed. The target FloorPlan need not be the branch's currently active one, but must not be soft-deleted. **Move is forbidden for any table participating in an active merge group (primary or secondary); the group must be Split first** (ADR-026) — no cascading move of a whole group. `Update Table` remains responsible only for a Table's own attributes and must never change `floorPlanId` - Move Table is the only operation that does. Move Table publishes `TableMoved` (Phase 8).

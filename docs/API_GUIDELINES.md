...
* **ADR-040 — Floor Plan Areas** (the concurrent halls inside one layout) are a collection nested under their FloorPlan: `POST`/`GET /restaurants/:restaurantId/branches/:branchId/floor-plans/:floorPlanId/areas` and `GET`/`PATCH`/`DELETE .../areas/:areaId`. They are the documented exception to the flat-individual-resource rule below: an area is only ever addressed from inside the editor that already holds its whole plan context, so a flat `/floor-plan-areas/:id` route would add a second tenant-resolution path to audit with no caller. Area-scoped table reads are the optional `floorPlanAreaId` filter on `GET .../floor-plans/:floorPlanId/tables`, not a fifth nesting level; an id that is not a live area of that plan returns `404` rather than silently widening to the whole plan. `PATCH /tables/:tableId` additionally accepts `floorPlanAreaId` and `color` (the floor editor's drag-to-save call writes them together with the layout fields), and `POST /tables/:tableId/move` accepts an optional `targetFloorPlanAreaId` resolved against the **target** plan. Authorization for all of these is the existing Owner/Admin stack — no new permission slug.
...
**Public Working Hours & Restaurant Phone Privacy (customer-facing correction):** two related corrections to the Discovery surface above, both additive/corrective (no route added or removed). (1) `GET /discovery/restaurants[/:restaurantId]` (list, detail, nearby, compare) now include `workingHours` (the Restaurant-level default schedule, `WorkingHours`), and `GET /discovery/restaurants/:restaurantId/branches[/:branchId]` now include each branch's own `workingHours` (its override schedule, `BranchWorkingHours`) - both batched (`ListWorkingHoursByRestaurantIdsUseCase`/`ListBranchWorkingHoursByBranchIdsUseCase`, one `IN (...)` query per response, no N+1), never merged with each other (no Branch-vs-Restaurant precedence is invented - `DATABASE_SCHEMA.md`'s "Branch Working Hours" section still documents that resolution as unowned). (2) The Branch responses on this same surface no longer include `phone` - private operational data, still returned unchanged by the authorized management endpoint (`GET /restaurants/:restaurantId/branches/:branchId`, Owner/Admin only). Both corrections replaced the Discovery module's direct reuse of the private `RestaurantResponseDto`/`BranchResponseDto`/`BranchListResponseDto` with dedicated customer-safe DTOs (`RestaurantPublicResponseDto`, `BranchPublicResponseDto`, `BranchPublicListResponseDto`, all under `modules/discovery/presentation/dto/`) - the same "dedicated public projection" precedent Phase 15.5's `.../floor-plan` correction (D11) already established, not a new pattern. `Restaurant` itself carries no phone/contact field of its own.

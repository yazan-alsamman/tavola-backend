...
**ADR-040 (Floor Plan Areas, implemented 2026-09-24) adds no event class.** Creating, updating and deleting a `FloorPlanArea` produces an audit-log entry only — `floor_plan_area.created`, `floor_plan_area.updated`, `floor_plan_area.deleted` (`targetType: FloorPlanArea`, `actorType: User`) — written directly through `AUDIT_LOG_WRITER`, exactly like the pre-existing `floor_plan.created`. There is no named FloorPlan event class in this document, no floor-plan room in the Phase 8 realtime allow-list, and no Phase 9 notification mapping, so no consumer exists to justify one. A Table's `floorPlanAreaId`/`color` changes ride the existing `TableCreated`/`TableUpdated` events with **unchanged payloads**; `TableMoved` likewise keeps its documented payload even though a move now reconciles area membership. Neither allow-list changes. If a consumer ever appears (e.g. realtime floor-plan sync for the editor), the upgrade path is the one Move Table already took in Phase 8 §6: promote the audit-only write to a real event class and add it to the allow-list in that phase's ADR.

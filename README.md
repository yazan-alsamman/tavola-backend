...
| Platform Features | Phases 9–14 | Notifications, Reviews, Offers, Subscriptions, Analytics (Phase 13 — Payments removed from product scope; see `TASKS.md`) |
...
Notification providers are abstracted (`NotificationProvider` port, ADR-007 Anti-Corruption Layer). Implemented (Phase 9, 2026-07-25): durable in-app notifications, OneSignal push delivery with ADR-025 Identity Verification, and the four Customer notification REST endpoints (`GET /notifications`, `GET /notifications/unread-count`, `PATCH /notifications/:id/read`, `PATCH /notifications/read-all`).
...
(Email removed from scope — 2026-07-25 product decision; Email is not a planned notification delivery channel. See TASKS.md's Phase 9 section.)
